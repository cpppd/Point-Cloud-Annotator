...
gulp.task('sync:ui', function () {
	return gulp.src('build/potree/**/*')
		.pipe(gulp.dest('../Point-Cloud-Annotator-UI/public/potree'));
});

// Define a build sequence to run tasks in order
gulp.task('dist', gulp.series('build', 'pack', 'sync:ui'));
...
gulp.task('watch', gulp.parallel("build", "pack", "webserver", async function () {
	...
	watch(watchlist, gulp.series("build", "pack", "sync:ui"));

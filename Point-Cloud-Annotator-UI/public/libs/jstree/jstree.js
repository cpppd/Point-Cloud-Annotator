  ...
  $.vakata.attributes = function (node, with_values) {
    ...
    if (node && node.attributes) {
      $.each(node.attributes, function (i, v) {
        if (
          $.inArray(v.name.toLowerCase(), ['style', 'contenteditable', 'hasfocus', 'tabindex']) !==
          -1
        ) {
          return;
        }
  ...
  $.jstree.plugins.checkbox = function (options, parent) {
    ...
    this._cascade_new_checked_state = function (id, checkedState) {
      ...
      if (
        ...
        var undetermined =
          selectedChildrenIds.length > 0 && selectedChildrenIds.length < node.children.length;
  ...
  $.jstree.plugins.types = function (options, parent) {
    ...
    this.get_json = function (obj, options, flat) {
      ...
      if ($.isArray(tmp)) {
        for (i = 0, j = tmp.length; i < j; i++) {
          tmp[i].type =
            tmp[i].id && m[tmp[i].id] && m[tmp[i].id].type ? m[tmp[i].id].type : 'default';
          if (options && options.no_id) {
            delete tmp[i].id;
    ...
    this.check = function (chk, obj, par, pos, more) {
      ...
      par = par && par.id ? par : this.get_node(par);
      var m =
          obj && obj.id ? (more && more.origin ? more.origin : $.jstree.reference(obj.id)) : null,
        tmp,
        d,
        i,
        j;

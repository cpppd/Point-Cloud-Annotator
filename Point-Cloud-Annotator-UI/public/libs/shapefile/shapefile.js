...
(function (global, factory) {
  ...
  var array_read = function () {
    ...
    this._array = null;
    return Promise.resolve(
      array ? { done: false, value: array } : { done: true, value: undefined },
    );
  };
  ...
  function slice(source) {
    return typeof source.slice === 'function'
      ? source
      : new SliceSource(typeof source.read === 'function' ? source : source.getReader());
  }
  ...
  var shapefile_read = function () {
    var that = this;
    return Promise.all([that._dbf ? that._dbf.read() : { value: {} }, that._shp.read()]).then(
      function (results) {
        var dbf = results[0],
          shp = results[1];
        return shp.done
  ...
  var shapefile = function (shpSource, dbfSource, decoder) {
    return Promise.all([shp(shpSource), dbfSource && dbf(dbfSource, decoder)]).then(
      function (sources) {
  ...
  function open(shp$$1, dbf$$1, options) {
    ...
    if (typeof shp$$1 === 'string') {
      ...
      if (dbf$$1 === undefined)
        dbf$$1 = path(shp$$1.substring(0, shp$$1.length - 4) + '.dbf', options).catch(
          function () {},
        );

...
ColladaLoader.prototype = Object.assign(Object.create(Loader.prototype), {
  ...
  parse: function (text, path) {
    ...
    function interpolate(key, prev, next, property) {
      ...
      key.value[property] =
        ((key.time - prev.time) * (next.value[property] - prev.value[property])) /
          (next.time - prev.time) +
        prev.value[property];
    }
              ...
              for (var i = 0; i < transforms.length; i++) {
                ...
                if (transform.sid && transform.sid.indexOf(jointIndex) !== -1) {
                  switch (joint.type) {
                    ...
                    case 'prismatic':
                      matrix.multiply(
                        m0.makeTranslation(axis.x * value, axis.y * value, axis.z * value),
                      );
                      break;
                  ...
                  switch (transform.type) {
                    ...
                    case 'translate':
                      matrix.multiply(
                        m0.makeTranslation(transform.obj.x, transform.obj.y, transform.obj.z),
                      );
                      break;

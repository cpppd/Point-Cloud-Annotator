...
var MMDLoader = (function () {
  ...
  MMDLoader.prototype = Object.assign(Object.create(Loader.prototype), {
    ...
    load: function (url, onLoad, onProgress, onError) {
      ...
      if (modelExtension !== 'pmd' && modelExtension !== 'pmx') {
        if (onError)
          onError(
            new Error('THREE.MMDLoader: Unknown model file extension .' + modelExtension + '.'),
          );
  ...
  MaterialBuilder.prototype = {
    ...
    _loadTexture: function (filePath, textures, params, onProgress, onError) {
      ...
      if (loader === null) {
        loader =
          filePath.slice(-4).toLowerCase() === '.tga' ? this._getTGALoader() : this.textureLoader;
      }
    ...
    _checkImageTransparency: function (map, geometry, groupIndex) {
      map.readyCallbacks.push(function (texture) {
        ...
        var imageData =
          texture.image.data !== undefined ? texture.image : createImageData(texture.image);
  ...
  AnimationBuilder.prototype = {
    ...
    buildCameraAnimation: function (vmd) {
      ...
      tracks.push(
        this._createTrack('target.position', VectorKeyframeTrack, times, centers, cInterpolations),
      );
      ...
      tracks.push(
        this._createTrack('.position', VectorKeyframeTrack, times, positions, pInterpolations),
      );

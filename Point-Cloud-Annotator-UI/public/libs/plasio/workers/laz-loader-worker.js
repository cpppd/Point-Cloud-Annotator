...
function handleEvent(msg) {
  switch (msg.type) {
    ...
    case 'header':
      if (!instance) throw new Error('You need to open the file before reading the header');
    ...
    case 'read':
      if (!instance) throw new Error('You need to open the file before trying to read');
      ...
      if (!o.header) throw new Error('You need to query header before reading');

      let h = o.header;

      var pointsToRead = Math.min(count * skip, h.pointsCount - o.readOffset);
      ...
      for (var i = 0; i < pointsToRead; i++) {
        ...
        if (i % skip === 0) {
          var a = new Uint8Array(Module.HEAPU8.buffer, buf_read, h.pointsStructSize);

          this_buf.set(a, pointsRead * h.pointsStructSize, h.pointsStructSize);

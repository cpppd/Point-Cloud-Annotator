(function (global, factory) {
  ...
  function Projection(srsCode, callback) {
    ...
    var datumObj =
      json.datum || datum(json.datumCode, json.datum_params, sphere_.a, sphere_.b, ecc.es, ecc.ep2);
      ...
      if (coords.length > 2) {
        if (
          ...
          if (typeof transformedArray.z === 'number') {
            return [transformedArray.x, transformedArray.y, transformedArray.z].concat(
              coords.splice(3),
            );
          } else {
  ...
  function init$6() {
    ...
    this.K =
      ...
      (Math.pow(Math.tan(0.5 * this.lat0 + FORTPI), this.C) * srat(this.e * sphi, this.ratexp));
  }
    ...
    if (this.sphere) {
      ...
      if (Math.abs(this.coslat0) <= EPSLN) {
        ...
        A =
          ...
          (this.cosX0 * (1 + this.sinX0 * sinX + this.cosX0 * cosX * Math.cos(dlon)));
  ...
  function forward$7(p) {
    ...
    var rotI = Math.atan(
      Math.sin(I) / (Math.sin(this.b0) * Math.tan(b) + Math.cos(this.b0) * Math.cos(I)),
    );

    var rotB = Math.asin(
      Math.cos(this.b0) * Math.sin(b) - Math.sin(this.b0) * Math.cos(b) * Math.cos(I),
    );
  ...
  function inverse$7(p) {
    ...
    var b = Math.asin(
      Math.cos(this.b0) * Math.sin(rotB) + Math.sin(this.b0) * Math.cos(rotB) * Math.cos(rotI),
    );
    var I = Math.atan(
      Math.sin(rotI) / (Math.cos(this.b0) * Math.cos(rotI) - Math.sin(this.b0) * Math.tan(rotB)),
    );
  ...
  function init$9() {
    ...
    if (this.no_off) {
      ...
      if (this.lat0 >= 0) {
        ...
        this.uc =
          ((-1 * this.al) / this.bl) * Math.atan2(Math.sqrt(dl * dl - 1), Math.cos(this.alpha));
      }
    }
  }
  ...
  function init$11() {
    ...
    this.k =
      (Math.tan(this.u0 / 2 + this.s45) / Math.pow(Math.tan(this.fi0 / 2 + this.s45), this.alfa)) *
      this.g;
    this.k1 = this.k0;
  ...
  function forward$10(p) {
    ...
    u =
      2 *
      (Math.atan((this.k * Math.pow(Math.tan(lat / 2 + this.s45), this.alfa)) / gfi) - this.s45);
    deltav = -delta_lon * this.alfa;
    s = Math.asin(
      Math.cos(this.ad) * Math.sin(u) + Math.sin(this.ad) * Math.cos(u) * Math.cos(deltav),
    );
  ...
  function inverse$10(p) {
    ...
    s =
      2 *
      (Math.atan(Math.pow(this.ro0 / ro, 1 / this.n) * Math.tan(this.s0 / 2 + this.s45)) -
        this.s45);
  ...
  var imlfn = function (ml, e0, e1, e2, e3) {
    ...
    for (var i = 0; i < 15; i++) {
      dphi =
        (ml -
          (e0 * phi - e1 * Math.sin(2 * phi) + e2 * Math.sin(4 * phi) - e3 * Math.sin(6 * phi))) /
  ...
  function inverse$11(p) {
    ...
    if (this.sphere) {
      ...
      var dsq = dl * dl;
      phi =
        phi1 - ((nl1 * Math.tan(phi1)) / rl1) * dl * dl * (0.5 - ((1 + 3 * tl1) * dl * dl) / 24);
    ...
    if (this.es > 0) {
      ...
      switch (this.mode) {
        ...
        case this.OBLIQ:
          ...
          this.dd =
            Math.cos(this.lat0) / (Math.sqrt(1 - this.es * sinphi * sinphi) * this.rq * this.cosb1);
  ...
  function inverse$12(p) {
    ...
    if (this.sphere) {
      ...
      lam =
        y === 0 && (this.mode === this.EQUIT || this.mode === this.OBLIQ) ? 0 : Math.atan2(x, y);
    } else {
      ab = 0;
      if (this.mode === this.OBLIQ || this.mode === this.EQUIT) {
        x /= this.dd;
      ...
      if (Math.abs(y + this.ml0) <= EPSLN) {
        ...
        for (i = MAX_ITER$2; i; --i) {
          ...
          dphi =
            ...
            ((this.es * Math.sin(2 * phi) * (ma * ma + bl - 2 * al * ma)) / (4 * cl) +
  ...
  function forward$23(p) {
    ...
    var q = asq + g;
    con =
      (Math.PI * this.R * (m * q - al * Math.sqrt((msq + asq) * (asq + 1) - q * q))) / (msq + asq);
    if (lat >= 0) {
  ...
  function forward$24(p) {
    ...
    if (this.sphere) {
      if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
        ...
        p.y =
          this.y0 + this.a * kp * (this.cos_p12 * sinphi - this.sin_p12 * cosphi * Math.cos(dlon));
        return p;
      }
    } else {
      e0 = e0fn(this.es);
    ...
    if (this.sphere) {
      ...
      if (Math.abs(rh) <= EPSLN) {
        ...
        if (Math.abs(con) <= EPSLN) {
          ...
          lon = adjust_lon(
            this.long0 +
              Math.atan2(p.x * sinz, rh * this.cos_p12 * cosz - p.y * this.sin_p12 * sinz),
          );
        }
      }
  ...
  function inverse$25(p) {
    ...
    lon = adjust_lon(
      this.long0 + Math.atan2(p.x * sinz, rh * this.cos_p14 * cosz - p.y * this.sin_p14 * sinz),
    );
    p.x = lon;
    p.y = lat;
    return p;

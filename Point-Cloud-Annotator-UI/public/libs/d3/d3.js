!(function () {
  ...
  d3.behavior.zoom = function () {
    ...
    function touchstarted() {
      ...
      function moved() {
        ...
        if (l1) {
          var distance1 =
              (distance1 = p1[0] - p0[0]) * distance1 + (distance1 = p1[1] - p0[1]) * distance1,
  ...
  d3.geo.path = function () {
    ...
    path.bounds = function (object) {
      d3_geo_pathBoundsX1 = d3_geo_pathBoundsY1 = -(d3_geo_pathBoundsX0 = d3_geo_pathBoundsY0 =
        Infinity);
  ...
  d3.layout.stack = function () {
    ...
    stack.order = function (x) {
      ...
      order =
        typeof x === 'function' ? x : d3_layout_stackOrders.get(x) || d3_layout_stackOrderDefault;
      return stack;
    };
    stack.offset = function (x) {
      if (!arguments.length) return offset;
      offset =
        typeof x === 'function' ? x : d3_layout_stackOffsets.get(x) || d3_layout_stackOffsetZero;
      return stack;
    };
    stack.x = function (z) {
  ...
  function d3_scale_linear(domain, range, interpolate, clamp) {
    ...
    function rescale() {
      var linear =
          Math.min(domain.length, range.length) > 2 ? d3_scale_polylinear : d3_scale_bilinear,
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    function scale(x) {
      return range[
        ((index.get(x) || (ranger.t === 'range' ? index.set(x, domain.push(x)) : NaN)) - 1) %
          range.length
      ];
    }
    ...
    scale.rangeRoundPoints = function (x, padding) {
      ...
      range = steps(
        start +
          Math.round(
            (step * padding) / 2 + (stop - start - (domain.length - 1 + padding) * step) / 2,
          ),
        step,
      );
      rangeBand = 0;
  ...
  var d3_category20 = [
    2062260, 11454440, 16744206, 16759672, 2924588, 10018698, 14034728, 16750742, 9725885, 12955861,
    9197131, 12885140, 14907330, 16234194, 8355711, 13092807, 12369186, 14408589, 1556175, 10410725,
  ].map(d3_rgbString);
  var d3_category20b = [
  ...
  var d3_category20c = [
    ...
    10607003, 13101504, 7695281, 10394312, 12369372, 14342891, 6513507, 9868950, 12434877, 14277081,
  ].map(d3_rgbString);
  ...
  function d3_svg_lineCardinalOpen(points, tension) {
    return points.length < 4
      ...
      : points[1] +
          d3_svg_lineHermite(points.slice(1, -1), d3_svg_lineCardinalTangents(points, tension));
  }
  ...
  function d3_svg_lineHermite(points, tangents) {
    ...
    if (quad) {
      path +=
        'Q' + (p[0] - (t0[0] * 2) / 3) + ',' + (p[1] - (t0[1] * 2) / 3) + ',' + p[0] + ',' + p[1];
      p0 = points[1];
      pi = 2;
    }
    ...
    if (quad) {
      var lp = points[pi];
      path +=
        'Q' + (p[0] + (t[0] * 2) / 3) + ',' + (p[1] + (t[1] * 2) / 3) + ',' + lp[0] + ',' + lp[1];
    }
    return path;
  }
  ...
  d3_transitionPrototype.each = function (type, listener) {
    ...
    if (arguments.length < 2) {
      ...
      d3_selection_each(this, function (node) {
        ...
        (transition.event || (transition.event = d3.dispatch('start', 'end', 'interrupt'))).on(
          type,
          listener,
        );
      });
